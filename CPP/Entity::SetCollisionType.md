...
int main(int argc, const char* argv[])
{
    ...
    ground->SetCollisionType(COLLISION_SCENE);
    ...
    //Create trigger zone
    auto zone = CreateSprite(world, 200, 200, false, 0, true);
    zone->SetPosition(100, 200);
    auto mtl = CreateMaterial();
    mtl->SetShaderFamily(LoadShaderFamily("Shaders/Unlit.json"));
    mtl->SetColor(0.5);
    mtl->SetTransparent(true);
    zone->SetMaterial(mtl);
    zone->SetCollisionType(COLLISION_TRIGGER);
    zone->RecordCollisions(true);
    ...
    while (window->Closed() == false and window->KeyDown(KEY_ESCAPE) == false)
    {
        ...
        //Change trigger zone color when collision occurs
        for (const auto& c : world->collisions)
        {
            if (c.entity[0] == zone or c.entity[1] == zone)
            {
                zone->SetColor(Random(), Random(), Random(), 0.5);
            }
        }
    }
    return 0;
